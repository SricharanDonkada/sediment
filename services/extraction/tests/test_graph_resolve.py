...
def test_resolve_exact_canonical_name_match():
    ...
    existing = [{"canonical_name": "Taco 007 Circulator", "aliases": [], "embedding": []}]
...
def test_resolve_exact_alias_match():
    ...
    existing = [{"canonical_name": "Taco 007 Circulator", "aliases": ["the 007", "007 pump"], "embedding": []}]
    new_clusters, matched = graph_resolve.resolve(mention_index, existing)
    assert new_clusters == []
    assert len(matched) == 1
    assert matched[0].canonical_name == "Taco 007 Circulator"
...
def test_resolve_matched_entity_excludes_already_known_aliases():
    ...
    existing = [{"canonical_name": "Taco 007 Circulator", "aliases": ["the 007"], "embedding": []}]
    _, matched = graph_resolve.resolve(mention_index, existing)
    assert matched[0].new_aliases == []


def test_resolve_no_existing_entities_creates_new_cluster():
...
# ── resolve: cosine similarity ────────────────────────────────────────────────

def test_resolve_cosine_above_threshold_matches_existing(monkeypatch):
    monkeypatch.setattr(
        graph_resolve.embed, "embed_document",
        lambda m: [1.0, 0.0],  # mention embedding
    )
    existing_embedding = [0.999, 0.001]  # cosine ≈ 0.9999 — above 0.92
    mention_index = {"Taco circulator": ["use the taco circulator"]}
    existing = [{"canonical_name": "Taco 007 Circulator", "aliases": [], "embedding": existing_embedding}]
...
def test_resolve_cosine_below_threshold_creates_new_cluster(monkeypatch):
    monkeypatch.setattr(
        graph_resolve.embed, "embed_document",
        lambda m: [0.0, 1.0],  # orthogonal to existing — cosine = 0.0
    )
...
def test_resolve_similar_unmatched_mentions_cluster_together(monkeypatch):
    monkeypatch.setattr(
        graph_resolve.embed, "embed_document",
        lambda m: [1.0, 0.0],  # all identical — cosine = 1.0
    )
    mention_index = {
        "Taco 007": ["quote1"],
        "007 pump": ["quote2"],
...
def test_resolve_dissimilar_unmatched_mentions_stay_separate(monkeypatch):
    ...
    monkeypatch.setattr(
        graph_resolve.embed, "embed_document",
        lambda m: embeddings.get(m, [0.5, 0.5]),
    )
...
def test_resolve_cluster_candidate_canonical_is_longest_mention(monkeypatch):
    monkeypatch.setattr(graph_resolve.embed, "embed_document", lambda m: [1.0, 0.0])
